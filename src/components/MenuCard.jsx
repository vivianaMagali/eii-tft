...
const MenuCard = ({ menu, orders, setOrders }) => {
  const [quantity, setQuantity] = useState(0);

  const addProduct = () => {
    setQuantity(quantity + 1);
    let updatedOrders = [...orders];
    const existingItem = updatedOrders.find((item) => item.name === menu.name);

    if (existingItem) {
      updatedOrders = updatedOrders.map((item) =>
        item.name === menu.name ? { ...item, amount: item.amount + 1 } : item,
      );
    } else {
      updatedOrders = [...orders, { ...menu, amount: 1 }];
    }
